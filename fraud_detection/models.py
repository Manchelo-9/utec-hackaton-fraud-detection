...
def train_xgboost(X_tr, y_tr, X_val, y_val):
    """Train XGBoost and return (model, val_predictions, auc)."""
...
def build_ensemble(val_preds: dict, aucs: dict, y_val):
    """Compute AUC-weighted ensemble on validation predictions.

    Returns (ensemble_val, weights_dict, ensemble_auc).
    """
    ...
    return ensemble_val, dict(zip(names, weights)), ensemble_auc
